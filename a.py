...
class CryptoApp:
    ...
    def _processar_pytorch(self, snapshots: dict) -> dict:
        """
        Processa indicadores técnicos, PyTorch forward/aprender para cada moeda.
        Roda completamente FORA do lock_universo.
        Retorna dict[moeda] = { campos_dado, sql, enviar_pulsos }
        """
        resultados = {}
        agora_dt = datetime.now()

        # Features de tempo — calculadas uma vez para todas as moedas
        tempo_s = agora_dt.hour * 3600 + agora_dt.minute * 60 + agora_dt.second
        ciclo_dia = (tempo_s / 86400) * 2 * math.pi
        hora_seno = math.sin(ciclo_dia)
        dia_seno  = math.sin((agora_dt.weekday() / 7) * 2 * math.pi)

        for moeda, snap in snapshots.items():
            try:
                preco   = snap["preco"]
                precos  = snap["precos"]
                ...
                sync_score = sum(
                    sync * sinal * 0.3
                    for sync, sinal in snap["cluster_previsoes"]
                )
                ...
                # ── PyTorch forward + delayed reward ──────────────────────────
                previsao = pesos.forward(features[:12])  # fallback se mente ausente
                mente    = self._garantir_mente(moeda)
                ...
                if mente is not None:
                    ...
                    if recompensas_maduras:
                        ...
                        try:
                            ...
                            print(f"[PyTorch] Erro aprendizado {moeda}: {e}")

                    # Verificação de acurácia real
                    self._atualizar_verificacao(moeda, preco, preds_percentual)

                    # Sinal consolidado
                    curto  = preds_percentual[:4]
                    medio  = preds_percentual[4:7]
                    longo  = preds_percentual[7:]
                    media_curto  = sum(curto) / len(curto)
                    media_medio  = sum(medio) / len(medio) if medio else media_curto
                    media_longo  = sum(longo) / len(longo) if longo else media_curto

                    consenso = (
                        (media_curto > 0.1 and media_medio > 0.05 and media_longo > 0) or
                        (media_curto < -0.1 and media_medio < -0.05 and media_longo < 0)
                    )
                    previsao = media_curto * (1.5 if consenso else 0.5)
                ...
                taxa   = 0.15 if acertou else 0.25
                energia_nova = snap["energia"] + recompensa * taxa
                energia_nova = max(5.0, min(10.0, energia_nova))  # tipo crypto

                # ── Pulsos para cluster ───────────────────────────────────────
                pulsos = []
                if abs(previsao) > 2.0 and snap["cluster_len"] > 1:
                    for sync, _ in snap["cluster_previsoes"]:
                        # Não temos o id do outro aqui — pulsos precisam do id
                        pass
                # Nota: pulsos de cluster precisam dos ids dos outros dados,
                # que não copiamos no snapshot para simplificar. Se quiser
                # reativar pulsos, adicione "cluster_ids" no snapshot.

                # ── Monta campos para escrever no dado ────────────────────────
                campos = {
                    "previsao": previsao,
                    "regime":   regime,
                    "rsi":      round(rsi, 2),
                    ...
                    "accuracy": round(pesos.accuracy, 3),
                    "ema9":     ema9,
                    "ema21":    ema21,
                    "sync":     round(sync_score, 3),
                    "price":    preco,
                    "delta":    preco - precos[-2],
                    "energia":  energia_nova,
                }

                if mente is not None:
                    campos["predicoes_array"]  = preds_percentual
                    campos["consenso_curto"]   = round(media_curto, 4)
                    campos["consenso_medio"]   = round(media_medio, 4)
                    campos["consenso_longo"]   = round(media_longo, 4)
                    for i, h in enumerate(HORIZONTES):
                        campos[f"previsao_{h}s"] = preds_percentual[i]

                resultados[moeda] = {
                    "campos_dado": campos,
                    "enviar_pulsos": pulsos,
                    "sql": {
                        "mente_id":   snap["id"],
                        "previsao":   previsao,
                        "preco_atual": preco,
                ...
                import traceback
                print(f"[CryptoApp] Erro processando {moeda}: {e}\n{traceback.format_exc()}")

        return resultados


    def _atualizar_verificacao(self, moeda: str, preco: float, preds: list):
        """Verifica acurácia real por horizonte — roda fora do lock."""
        if moeda not in self.verificacoes:
            self.verificacoes[moeda] = deque(maxlen=5000)
            self.resultados_verificacao[moeda] = {
                h: {"acertos": 0, "erros": 0, "total": 0} for h in HORIZONTES
            }
        ...
        for v in self.verificacoes[moeda]:
            ...
            for i, horizonte in enumerate(HORIZONTES):
                ...
                if tempo_passado >= horizonte and not v.get(chave):
                    preco_alvo = next(
                        (v2["price"] for v2 in self.verificacoes[moeda]
                        if abs(v2["time"] - (v["time"] + horizonte)) <= self.timeframe * 2),
                        preco
                    )
                    dir_prevista = 1 if v["preds"][i] > 0 else -1
                    dir_real     = 1 if preco_alvo > v["price"] else -1
                    chave_res    = "acertos" if dir_prevista == dir_real else "erros"
                    resultados[horizonte][chave_res] += 1
                    resultados[horizonte]["total"]   += 1
                    v[chave] = True
    ...
    def loop_api(self):
        ...
        while self.rodando_api:
            try:
                print(f"🔄 [DEBUG] Tick loop_api - moedas: {list(self.ids_cripto.keys())}")
                todos_precos = pegar_todos_precos()
                print(f"🔄 [DEBUG] Preços obtidos: {len(todos_precos)} moedas") 
                if todos_precos:
                ...
                # 1) Extrai cópias seguras dentro do lock — rápido, sem PyTorch
                snapshots = {}
                with lock_universo:
                    clusters_universo = self.universo.detectar_clusters()
                    cluster_por_id = {}
                    ...
                    for moeda, id_dado in self.ids_cripto.items():
                        ...
                        # Atualiza history e candles ainda dentro do lock (escrita no dado)
                        dado["history"].append({"time": int(time.time()), "price": preco})
                        if len(dado["history"]) > 200:
                            dado["history"].pop(0)

                        if len(dado["history"]) < 26:
                            dado["price"] = preco
                            continue

                        self._atualizar_candles(dado, preco)

                        minha_cluster = cluster_por_id.get(id_dado, [])
                        energia_cluster = sum(d["energia"] for d in minha_cluster if d["id"] != id_dado)

                        # Copia tudo que o PyTorch vai precisar — sem referências ao dado original
                        snapshots[moeda] = {
                            "id": id_dado,
                            "preco": preco,
                            "precos": [p["price"] for p in dado["history"]],
                            "candles": list(dado["candles"]),
                            "energia": dado.get("energia", 5.0),
                            "energia_cluster": energia_cluster,
                            "cluster_previsoes": [
                                (self.universo.sincronizacao(dado, o), o.get("previsao", 0))
                                for o in minha_cluster if o["id"] != id_dado
                            ],
                            "memoria_score": self.universo.score_memoria(dado),
                            "delta_energia": self.universo.memoria_global.get("delta_energia", 0),
                            "cluster_len": len(minha_cluster),
                        }

                # 2) Todo processamento pesado FORA do lock
                resultados = self._processar_pytorch(snapshots)

                # 3) Escreve resultados de volta — rápido
                with lock_universo:
                    for moeda, res in resultados.items():
                        id_dado = self.ids_cripto.get(moeda)
                        dado = next((d for d in self.universo.dados if d["id"] == id_dado), None)
                        if dado is not None:
                            dado.update(res["campos_dado"])
                            if res.get("enviar_pulsos"):
                                for outro_id, energia_pulso in res["enviar_pulsos"]:
                                    self.universo.enviar_pulso(id_dado, outro_id, energia=energia_pulso)

                # SQL em background — nunca dentro do lock
                for moeda, res in resultados.items():
                    if "sql" in res:
                        s = res["sql"]
                        self._executor.submit(
                            self._registrar_sql_background,
                            s["mente_id"], moeda, s["previsao"], s["preco_atual"],
                            s["acertou"], s["reward"], s["regime"]
                        )

                agora = time.time()
                if agora - ultimo_salvar > 120:
    ...
    def _atualizar_candles(self, dado: dict, preco: float):
        """Atualiza candles no dado — deve ser chamado dentro do lock."""
        if "candles" not in dado:
            dado["candles"] = []
        ...
        if not dado["candles"] or dado["candles"][-1]["time"] != candle_time:
            dado["candles"].append({
                "time": candle_time, "open": preco,
                "high": preco, "low": preco, "close": preco
            })
        else:
            c = dado["candles"][-1]
            c["high"]  = max(c["high"], preco, c["open"], c["close"])
    ...
    def _registrar_sql_background(self, mente_id, symbol, previsao,
                                preco_atual, acertou, reward, regime):
        """Tenta registrar no SQL - NUNCA trava o sistema"""
        try:
            from Software.core.mind_sql import get_banco_sql
            banco_sql = get_banco_sql()
            if banco_sql is None:
                return
            banco_sql.registrar_performance(
                mente_id=mente_id, symbol=symbol, previsao=previsao,
                preco_atual=preco_atual, direcao=1 if previsao > 0 else -1,
                acertou=acertou, reward=reward,
                loss=abs(previsao - (1 if acertou else -1)) * 0.1,
                regime=regime
            )
        except Exception as e:
            erro_msg = str(e).lower()
            # ⭐ Só loga se NÃO for erro de "locked"
            if "locked" not in erro_msg and "database" not in erro_msg:
                print(f"[SQL] Erro: {e}")